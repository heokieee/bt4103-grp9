...
# These are generated by retrain.py and uploaded to GCS after each retrain.
# They are optional — the app works without them on first deploy before any retrain.
...
def sync_artifacts_from_bucket() -> tuple[bool, str]:
    ...
    # Required artifacts — app cannot start without these.
    ...
    # Optional artifacts — generated by retrain.py. Skip silently if not in bucket yet.
    for local_path in OPTIONAL_ARTIFACT_PATHS:
        blob_name = build_blob_name(bucket_prefix, local_path.name)
        blob = bucket.blob(blob_name)
        if not blob.exists(client):
            continue
